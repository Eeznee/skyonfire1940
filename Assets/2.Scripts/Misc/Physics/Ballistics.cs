...
public static class Ballistics
{
    ...
    public static float InterceptionTime(Vector3 relativeProjectileVelocity, Vector3 relativePos, Vector3 relativeVel)
    {
        float velocitySquared = relativeVel.sqrMagnitude;
        if (velocitySquared < 0.001f)
            return 0f;

        float a = velocitySquared - relativeProjectileVelocity.sqrMagnitude;
    ...
    public static float PerfectTimeToTarget(SofObject shooter, Vector3 muzzlePosition, SofObject target, float muzzleVelocity, float bulletDragCoeff)
    {
        Vector3 relativeTarget = target.tr.position - muzzlePosition;
        float distance = relativeTarget.magnitude;
        Vector3 targetDir = relativeTarget / distance;
        Vector3 relativeVel = target.rb.linearVelocity - shooter.rb.linearVelocity;

        float roughTimeToTarget = distance / (muzzleVelocity * 0.8f);
        float roughPredictionDistance = (relativeTarget + relativeVel * roughTimeToTarget).magnitude;
        Vector3 worldProjectileVelocity = muzzleVelocity * targetDir + shooter.rb.linearVelocity;

        float averageWorldProjectileSpeed = AverageProjectileVelocity(roughPredictionDistance, worldProjectileVelocity.magnitude, bulletDragCoeff);
        Vector3 relativeAverageProjectVelocity = averageWorldProjectileSpeed * targetDir - shooter.rb.linearVelocity;

        return InterceptionTime(relativeAverageProjectVelocity, relativeTarget, relativeVel);
    }
    private static Vector3 LeadWithoutBulletDrag(SofObject shooter, SofObject target, float timeToTarget)
    {
        Vector3 lead = timeToTarget * target.rb.linearVelocity;
        lead += -shooter.rb.linearVelocity * timeToTarget;
        lead += 0.5f * timeToTarget * timeToTarget * -Physics.gravity;
        return lead;
    }
    public static Vector3 PerfectLead(SofObject shooter, Vector3 muzzlePosition, SofObject target, float muzzleVelocity, float bulletDragCoeff)
    {
        float timeToTarget = PerfectTimeToTarget(shooter, muzzlePosition, target, muzzleVelocity, bulletDragCoeff);
        Vector3 lead = LeadWithoutBulletDrag(shooter, target, timeToTarget);

        Vector3 leadDir = (target.tr.position + lead - muzzlePosition).normalized;
        Vector3 bulletVelocity = muzzleVelocity * leadDir + shooter.rb.linearVelocity;
        float bulletSpeed = bulletVelocity.magnitude;

        float noDragDistance = bulletSpeed * timeToTarget;
        float withDragDistance = BallisticDistance(bulletSpeed, bulletDragCoeff, timeToTarget);

        Vector3 dragOffset = (noDragDistance - withDragDistance) * bulletVelocity.normalized;
        lead += Vector3.ProjectOnPlane(dragOffset, leadDir);
