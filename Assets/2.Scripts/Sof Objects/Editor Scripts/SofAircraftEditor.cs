...
public class SofAircraftEditor : SofComplexEditor
{
    ...
    SerializedProperty pitchBoost;
    SerializedProperty rollBoost;
    ...
    protected override void OnEnable()
    {
        ...
        pitchBoost = serializedObject.FindProperty("pitchBoost");
        rollBoost = serializedObject.FindProperty("rollBoost");
    ...
    static bool showStations = true;
    static bool showStationsSelection = true;
    public override void OnInspectorGUI()
    {
        ...
        showStations = EditorGUILayout.Foldout(showStations, "Stations Loadout", true, EditorStyles.foldoutHeader);
        if (showStations)
        {
            EditorGUI.indentLevel++;
            ...
            showStationsSelection = EditorGUILayout.Foldout(showStationsSelection, "Editor Selection", true, EditorStyles.foldoutHeader);
            if (showStationsSelection)
            {
                EditorGUI.indentLevel++;
                foreach (Station station in aircraft.Stations)
                    station.picked = EditorGUILayout.IntSlider(station.name + "   :   " + station.OptionName(), station.picked, 0, station.loadouts.Length - 1, GUILayout.MaxWidth(600f));
        ...
        if (showControlAxes)
        {
            ...
            EditorGUILayout.Slider(pitchBoost, 1f, 8f, new GUIContent("Pitch Boost"));
            EditorGUILayout.Slider(rollBoost, 1f, 8f, new GUIContent("Roll Boost"));
