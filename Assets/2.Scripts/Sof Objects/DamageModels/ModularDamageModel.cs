...
public class ModularDamageModel : SofDamageModel
{
    ...
    public override void Explosion(Vector3 center, float tnt)
    {
        ...
        foreach (SofModule m in complex.modules.ToArray()) if (m) m.ExplosionDamage(center, tnt, out float dmg, out float hole);
    }
    public override HitResult ProjectileRaycast(Vector3 position, Vector3 velocity, ProjectileChart chart)
    {
        if(rb) velocity -= rb.linearVelocity;
        ...
        foreach (RaycastHit hit in hits)
        {
            ...
            float armor = Random.Range(0.8f, 1.2f) * module.Armor(hit.collider).surfaceArmor / Mathf.Cos(alpha * Mathf.Deg2Rad);


            if (penetrationPower > armor)//If penetration occurs
            {
                module.ProjectileDamage(chart.KineticDamage(sqrVelocity), chart.diameter, chart.fireChance, hit.collider);
                armor += Random.Range(0.8f, 1.2f) * module.Armor(hit.collider).fullPenArmor;
