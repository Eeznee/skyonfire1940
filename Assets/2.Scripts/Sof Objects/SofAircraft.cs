...
public class SofAircraft : SofComplex
{
    ...
    public List<int> stationsSelection;
    ...
    [SerializeField] private float controlsForceBoost = 1f;
    [SerializeField] private float stallMarginAngle = 1f;
    [SerializeField] private float pitchBoost = 1f;
    ...
    public float ControlsForceBoost => controlsForceBoost;
    public float StallMarginAngle => stallMarginAngle;
    public float PitchBoost => pitchBoost;
    public float RollBoost => rollBoost;
    ...
    {
        if(Stations != null) foreach (Station s in Stations) s?.SelectAndDisactivate();
    ...
    protected override void InitializePhysics()
    {
        ...
        if (!GroundedStart) rb.linearVelocity = transform.forward * stats.MaxSpeed(data.altitude.Get, 1f);
    ...
    {
        SofAirframe collided = col.GetContact(0).thisCollider.transform.GetComponentInParent<SofAirframe>();
    ...
    protected override void FixedUpdate()
    {
        ...
        {
            controls.SetTargetInput(new AircraftAxes(0f, 0f, 0f), true, PitchCorrectionMode.Raw);
        }

        if (transform.position.y < 500f && data.grounded.Get) timeSinceLastLanding = 0f;
