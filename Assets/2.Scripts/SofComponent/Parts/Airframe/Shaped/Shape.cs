...
public struct Shape
{
    ...
    public Quad ToQuad()
    {
        ...
        Vector3 tipLiftPos = rootLiftPos + (localRight * scale.x) + (localForward * Mathf.Abs(scale.x) / Mathf.Tan((90f - sweep) * Mathf.Deg2Rad));
    ...
    Vector3 SnapPosition(Shape snapTo)
    {
        ...
        worldPos += snapTo.forward * Mathf.Abs(snapTo.scale.x) / Mathf.Tan((90f - snapTo.sweep) * Mathf.Deg2Rad);
