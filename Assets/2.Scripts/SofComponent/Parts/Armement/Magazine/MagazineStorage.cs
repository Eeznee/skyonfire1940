...
public class MagazineStorage : SofPart
{
    ...
    {
        return Application.isPlaying ? magsLeft * magRef.LoadedMass() : Mathf.Min(positions.Length, localRotations.Length) * magRef.LoadedMass();
    }
    ...
    public Magazine GetMag()
    {
        if (true)
        {
            ...
            mag.InitializeComponent(complex);
            magsLeft--;
            return mag;
        }
    }
