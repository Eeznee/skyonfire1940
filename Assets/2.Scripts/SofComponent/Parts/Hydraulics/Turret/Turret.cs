...
#endif
abstract public class Turret : SofComponent
{
    //General Parameters ---------------------------------------------------------------------------------------------------------------
    ...
    public bool PilotMainGun() { return guns[0].controller != GunController.Gunner; }
    ...
    public void Operate(bool fire,bool automaticPrevention)
    {
        ...
        if (preventSelfDamage || automaticPrevention)
        {
            for (int j = 0; j < guns.Length; j++)
                if (Physics.Raycast(guns[j].transform.position, guns[j].transform.forward, 10f, LayerMask.GetMask("SofComplex"))) return;
        }
