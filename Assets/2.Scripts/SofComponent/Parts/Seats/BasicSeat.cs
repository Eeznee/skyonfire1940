...
public class BasicSeat : SofComponent
{
    ...
    {
        Transform pov = player && CameraInputs.zoomed ? zoomedPOV : defaultPOV;
        if (pov.root == transform.root) return pov.position;
        else return transform.position + data.up.Get * 0.75f;
    }
    ...
    {
        headLookDirection = Player.seatInterface == SeatInterface.Gunner ? SofCamera.directionInput : SofCamera.tr.forward;
    }
...
public class BasicSeatEditor : Editor
{
    public override void OnInspectorGUI()
    {
        ...
        seat.externalViewPoint = EditorGUILayout.Vector3Field("External View Point", seat.externalViewPoint);
        seat.goProViewPoint = EditorGUILayout.Vector3Field("Go Pro View Point", seat.goProViewPoint);
