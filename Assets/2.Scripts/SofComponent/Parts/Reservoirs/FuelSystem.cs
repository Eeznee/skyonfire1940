...
{   
    ...
    public FuelSystem(SofComplex _complex)
    {
        complex = _complex;
        totalCapacity = 0f;
        ...
        {
            if (liquidTank.liquid.type == LiquidType.Fuel)
            {
        ...
        if (complex.aircraft)
        {
            EnginePreset engine = complex.aircraft.engines[0].preset;
            float c = engine.ConsumptionRate(1f, engine.gear1.Evaluate(2000f) * 745.7f);
            fullThrottleCons = c * complex.aircraft.engines.Length;
        }
    }
    ...
    static int SortByPosition(LiquidTank f1, LiquidTank f2)
    {
        ...
        if (Mathf.Abs(f1LocalPos.x) > Mathf.Abs(f2LocalPos.x) + 1f) return 1;
        else if (Mathf.Abs(f1LocalPos.x) < Mathf.Abs(f2LocalPos.x) - 1f) return -1;

        float f1DistanceCog = (f1.complex.GetCenterOfMass() - f1LocalPos).magnitude;
        float f2DistanceCog = (f1.complex.GetCenterOfMass() - f2LocalPos).magnitude;
