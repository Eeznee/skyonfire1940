...
public static class AircraftControl
{
    ...
    public static Vector3 TrackingInputs(Vector3 target, SofAircraft ctr, float targetRoll, float levelingFactor, bool assist)
    {
        ...
        if (ctr.data.ias.Get < 5f) return axis;
        if (assist)
        {
        ...
        axis.x *= Mathv.SmoothStart(Mathf.Min(1f, ctr.cruiseSpeed / ctr.data.ias.Get), 2);
        return axis;
    }
    ...
    public static void PlayerUpdate(SofAircraft aircraft)
    {
        ...
        {
            float thr = Player.aircraft.engines[0].throttleInput;
    ...
    public static void PlayerFixed(SofAircraft aircraft)
    {
        ...
        if (GameManager.Controls() == ControlsMode.Tracking) //Tracking input, mouse
        {
            ...
            axis = TrackingInputs(targetPos, aircraft, 0f, PlayerActions.dynamic ? 0f : 1f, false);
        ...
        else //Direct input, joystick, phone
        {
            ...
            axis.y = -actions.Rudder.ReadValue<float>();
            aircraft.SetControls(axis, !GameManager.fullElevator, false);
        }
    }
}
