...
public class ObjectData
{
    ...
    {
        gsp = new Value<float>(() => { return rb.linearVelocity.magnitude; }, this);
        vsp = new Value<float>(() => { return rb.linearVelocity.y; }, this);
        tas = new Value<float>(() => { return gsp.Get; }, this);
        signedTas = new Value<float>(() => { return tas.Get * Mathf.Sign(Vector3.Dot(rb.transform.forward, rb.linearVelocity)); }, this);
        ...
        angleOfSlip = new Value<float>(() => { return Vector3.SignedAngle(tr.forward, Vector3.ProjectOnPlane(rb.linearVelocity, tr.up), tr.up); }, this);
        angleOfAttack = new Value<float>(() => { return tas.Get < 2f ? 0f : Vector3.SignedAngle(tr.forward, Vector3.ProjectOnPlane(rb.linearVelocity, tr.right), tr.right); }, this);
    ...
    {
        if (rb) prevVel = rb.linearVelocity;
    }
    ...
    protected void ComputeAcceleration()
    {
        ...
        //Acceleration
        acceleration = (rb.linearVelocity - prevVel) * TimeManager.invertFixedDelta;
