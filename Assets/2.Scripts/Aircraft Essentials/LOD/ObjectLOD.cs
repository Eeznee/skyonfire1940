...
public class ObjectLOD : SofComponent
{
    ...
    /*
    const float lod0Limit = 0.8f;
    const float lod1Limit = 0.6f;
    const float lod2Limit = 0.45f;
    */
    const float lod0Limit = 0.2f;
    const float lod1Limit = 0.1f;
    ...
    public void ResetLODGroup()
    {
        ...
        if (lods.Length != 4) lods = new LOD[] { new LOD(lod0Limit, null), new LOD(lod1Limit, null), new LOD(lod2Limit, null), new LOD(0f, null) };
