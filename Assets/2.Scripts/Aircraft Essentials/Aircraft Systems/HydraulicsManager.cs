...
public class HydraulicsManager
{
    ...
    public void SetAllHydraulicsToDefaultPosition()
    {
        HydraulicSystem[] hydraulics = aircraft.GetComponentsInChildren<HydraulicSystem>();

        foreach(HydraulicSystem hydraulic in hydraulics)
        {
            hydraulic.SetInstant(hydraulic.DefaultState);
        }
        aircraft.animator.Update(100f);
    }
    ...
    public void SetFlapsRaw(float target)
    {
        if (!flaps) return;
        flaps.Set(target);
        Log.Print(flaps.GetLogStateInput("Flaps", "Unoperational"), "flaps");
    }
    public void SetGear()
    {
