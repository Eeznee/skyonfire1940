...
public class PilotSeat : CrewSeat
{
    ...
    {
        //Post Maneuver
        aircraft.SetThrottle(Mathf.Min(aircraft.throttle, Mathf.InverseLerp(aircraft.maxSpeed * 0.9f, aircraft.maxSpeed * 0.7f, data.ias)));
        ...
        if (maneuver == null || !maneuver.MaxPitch())
        {
            ...
            aircraft.controlTarget.x = Mathf.Min(aircraft.controlTarget.x, crew.body.Stamina());
        }
    }
    private void Shooting(AI.GeometricData bfmData)
    {
        ...
        float minAngle = target.wingSpan / bfmData.distance * (1f + difficulty * 0.5f) * Mathf.Rad2Deg;
        ...
        if (fire)//Spray target and auto aim guns
        {
            ...
            foreach (SofAircraft a in GameManager.squadrons[aircraft.squadronId])
            {
                ...
                if (Mathf.Abs(localPos.y) < a.wingSpan / 2f && Mathf.Abs(localPos.x) < a.wingSpan && localPos.z > 0f) return;
            }
    ...
    {
        foreach (Engine e in aircraft.engines) if (!e.Working() && e.Operational()) e.Set(true, false);
    }
