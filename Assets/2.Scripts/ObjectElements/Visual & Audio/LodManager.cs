...
public class LodManager : ObjectElement
{
    ...
    {
        lod = newLod;
        if (OnSwitchEvent != null) OnSwitchEvent(newLod);
    }
    void Update()
    {
        float dis = (data.position.Get - PlayerCamera.camPos).sqrMagnitude;
        ...
        if (newLod < 3)
        {
            Bounds bounds = new Bounds(data.position.Get, Vector3.one * 10f);
