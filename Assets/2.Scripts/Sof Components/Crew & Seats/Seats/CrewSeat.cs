...
public class CrewSeat : SofComponent
{
    ...
    [Range(0f,0.3f)]
    public float lookBehindRightShift = 0f;
    [Range(0f, 0.3f)]
    public float lookBehindLeftShift = 0f;
    ...
    public void TryReload(bool onlyEmptyGuns)
    {
        ...
        foreach (Gun gun in reloadableGuns)
        {
            ...
            if ((onlyEmptyGuns && gun.magazine.ammo > 0) && !gun.MustBeCocked) continue;
    ...
    protected IEnumerator Reload(Gun gun)
    {
        ...
        if (!noMagChange)
        {
            ...
            while (count > 0f)//Animate
            {
                ...
                newMag.localPosition = gun.magazineAttachPoint +  projectedVector * projected.Integral(count) + lateralVector * lateral.Integral(count) + ejectVector * eject.Integral(count);
                yield return null;
            }
