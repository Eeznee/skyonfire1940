...
public class PilotSeat : CrewSeat
{
    ...
    public override void PlayerUpdate(CrewMember crew)
    {
        ...
        ControlsManager.PilotUpdateAxes(crew);
    }

    public void PlayerPilotAircraft(CrewMember crew)
    {
        ControlsManager.PilotFixedUpdateAxes(crew);
    }
    ...
    private void Shooting(AI.GeometricData bfmData)
    {
        ...
        Vector3 perfectLead = Ballistics.PerfectLead(sofObject, sofObject.tr.position, target, ammoReference.defaultMuzzleVel, ammoReference.DragCoeff);
        ...
        if ((bfmData.dir + perfectLead).sqrMagnitude > trueRange * trueRange) return;

        float gunsAngle = Vector3.Angle(bfmData.dir + perfectLead, transform.root.forward);
        ...
        Vector3 spray = xSpray * target.stats.wingSpan * 0.65f * targetTr.right;
        spray += ySpray * target.stats.wingSpan * 0.3f * targetTr.up;

        aircraft.armament.CheatPointGuns(targetTr.position + perfectLead + spray);
        aircraft.armament.FireGuns(true,true, 0.25f);
    }
