...
public class GunnerSeat : CrewSeat
{
    ...
    public override void PlayerUpdate(CrewMember crew)
    {
        ...
        {
            float input = ControlsManager.gunner.Hydraulics.ReadValue<float>();
            progressiveHydraulic.SetDirection(Mathf.RoundToInt(input));
        }
        if (!gunMount) return;
        if (!handsBusy) NewGrips(defaultRightHand, defaultLeftHand);

        float x = ControlsManager.gunner.RotateHorizontal.ReadValue<float>();
        float y = ControlsManager.gunner.RotateVertical.ReadValue<float>();
        float special = ControlsManager.gunner.SpecialAxis.ReadValue<float>();
        ...
        if (ControlsManager.CurrentMode() == ControlsMode.Tracking && !gunMount.ForceJoystickControls)
            ...
            gunMount.OperateMainManual(new Vector2(x,y));

        gunMount.OperateSpecialManual(special);

        bool firing = ControlsManager.gunner.Fire.ReadValue<float>() > 0.5f;
    ...
    {
        //PERFECT LEAD
        float t = Ballistics.PerfectTimeToTarget(sofObject, gunMount.tr.position, target, gunMount.Ammunition.defaultMuzzleVel, gunMount.Ammunition.DragCoeff);
        Vector3 gravityLead = 0.5f * t * t * -Physics.gravity;
        Vector3 velocityLead = t * target.rb.linearVelocity - gunMount.rb.linearVelocity * t;
        Vector3 perfectLead = velocityLead + gravityLead;

        Vector3 targetDirection = target.tr.position - gunMount.tr.position;
        Vector3 targetVel = target.rb.linearVelocity;
        ...
        Vector3 inaccurateLead = velocityInaccuracy * velocityLead + gravityInaccuracy * gravityLead - gunMount.rb.linearVelocity * t;
        ...
        float fuzeInaccuracy = 100f + Mathf.Abs(Vector3.Dot(gunMount.FiringDirection, target.rb.linearVelocity));
