...
public class Gun : SofComponent, IMassComponent
{
    ...
    public Vector3 magazineAttachPoint;
    ...
    public Quaternion ConvergedRotation => tr.rotation * (cheatTime > Time.time ? cheatConvergence : convergence);

    public Vector3 WorldMuzzlePos => tr.TransformPoint(bulletPos);
    ...
    public override void Initialize(SofModular _complex)
    {
        ...
        if (magazine) magazineAttachPoint = transform.InverseTransformPoint(magazine.tr.position);
    ...
    public void Trigger(float triggerTime)
    {
        ...
        {
            StartCoroutine(TriggerCycle(triggerTime));
        }
        triggerTimer = triggerTime;
    }
    public IEnumerator TriggerCycle(float triggerTime)
    {
    ...
    private void FireBullet(float delay)
    {
        ...
        Quaternion bulletRotation = ConvergedRotation;
        ...
        bullet.transform.position = WorldMuzzlePos;
        bullet.gameObject.SetActive(true);

        Vector3 velocity = bulletDirection * bullet.properties.baseVelocity + rb.linearVelocity;

        bullet.InitializeTrajectory(velocity, sofModular.damageModel);
        if (fuzeDistance > 0.1f) bullet.SetFuzeBasedOnDistance(fuzeDistance + Random.Range(-20f,20f));
    }
    ...
    public bool MustBeCocked => mechanism.MustBeCocked();
    public bool MustBeReloaded => !reloading && (MustBeCocked || !magazine || magazine.ammo <= 0);
    public bool CanBeReloaded => !reloading && (MustBeCocked || !magazine || magazine.ammo < magazine.capacity);
    public bool Firing() { return mechanism.IsFiring(); }
    public Vector3 MagazinePosition() { return transform.TransformPoint(magazineAttachPoint); }
}
