...
public class GunsGroupAudio : MonoBehaviour
{
    ...
    IEnumerator AudioCycle()
    {
        ...
        do
        {
            if (avm.sofModular == Player.modular)
            {
                ...
                foreach (Gun gun in guns)
                {
                    if (gun && !gun.gunPreset.singleShotsAudio && gun.Firing())
                    {
                        ...
                        float sqrDistance = (Player.crew.tr.position - gun.tr.position).magnitude;
                        totalVibrations += vibrationsPower / Mathf.Max(sqrDistance, 1f);
                    }

                    totalVibrations = Mathf.Log(totalVibrations + 1f, 10f);
                }
                VibrationsManager.SendVibrations(totalVibrations, 0.15f);
            }
    ...
    {
        if (sample.endOut) avm.PlayAudioClip(sample.endOut, 1f, SofAudioGroup.External, true);
        if (sample.endIn) avm.PlayAudioClip(sample.endIn, 1f, SofAudioGroup.Cockpit, false);
    }
