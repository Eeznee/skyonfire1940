...
public class Wheel : SofModule, IMassComponent
{
    ...
    {
        return ModulesHPData.WheelArmor;
    }
    ...
    void FixedUpdate()
    {
        ...
        else
        {
            ...
            if (Player.modular == sofModular)
            {
                bool averageHits = true;

                Vector3 secondRayOrigin = rayOrigin + rb.linearVelocity * 0.3f;
                averageHits &= Physics.Raycast(secondRayOrigin, Vector3.down, out RaycastHit secondHit, 10f, mask);

                Vector3 thirdOrigin = rayOrigin - rb.linearVelocity * 0.3f;
                averageHits &= Physics.Raycast(thirdOrigin, Vector3.down, out RaycastHit thirdHit, 10f, mask);

                if (averageHits)
                {
                    hit.point = 0.33333f * (hit.point + secondHit.point + thirdHit.point);
                    hit.normal = 0.33333f * (hit.normal + secondHit.normal + thirdHit.normal);
                    hit.distance = 0.33333f * (hit.distance + secondHit.distance + thirdHit.distance);
                }
            }
            ...
            if (grounded) GroundedFixedUpdate(hit);
        }
    }
