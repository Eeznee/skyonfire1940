...
public class EnginesGroupAudio : MonoBehaviour
{
    ...
    {
        if (engine.Class == EngineClass.JetEngine) avm.PlayAudioClip(preset.IgnitionClip, 1f, SofAudioGroup.Persistent, true);

        else StartCoroutine(IgnitionCoroutine(engine));
    }
    public IEnumerator IgnitionCoroutine(Engine engine)
    {
        avm.PlayAudioClip(preset.PreIgnitionClip, 1f, SofAudioGroup.Persistent, true);

        while (engine.RadPerSec < PistonEngine.preIgnitionRadPerSec)
        {
            yield return null;
        }
        avm.PlayAudioClip(preset.IgnitionClip, 1f, SofAudioGroup.Persistent, true);
    }
