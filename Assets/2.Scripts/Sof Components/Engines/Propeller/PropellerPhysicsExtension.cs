...
public partial class Propeller : SofModule, IMassComponent, IAircraftForce
{
    ...
    const float functionOverallFactor = 0.1f;
    ...
    public float MotionTorque(float bladeAngle, float radPerSec, float signedTas)
    {
        ...
        return -Mathf.Sign(radPerSec) * BladeSpeedSquared(radPerSec, data.tas.Get) * cd * data.relativeDensity.Get * radius / engine.PistonPreset.PropellerReductionGear;
    }
