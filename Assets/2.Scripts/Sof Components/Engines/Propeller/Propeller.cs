...
public partial class Propeller : SofModule, IMassComponent, IAircraftForce
{
    ...
    public ForceAtPoint SimulatePointForce(FlightConditions flightConditions)
    {
        ...
        if (engine.Igniting) direction = Quaternion.AngleAxis(invertRotation ? -12f : 12f, aircraft.tr.up) * direction;
    ...
    public override void ExplosionDamage(Vector3 center, float tnt, out float hole, out float damage)
    {
        hole = damage = 0f;
        //Propellers cannot take damage from explosions or bullets
    }
