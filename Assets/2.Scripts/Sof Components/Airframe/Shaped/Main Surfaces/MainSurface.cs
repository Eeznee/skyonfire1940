...
public abstract class MainSurface : ShapedAirframe, IAircraftForce
{
    ...
    public override Vector2 SimulatedCoefficients(float angleOfAttack, float pointSpeed, AircraftAxes axes)
    {
        ...
        angleOfAttack -= ControlSurfaceEffect(axes);

        //Flaps Effect
        Vector2 coeffs;
        if (HasAircraft && hasFlaps && aircraft.hydraulics.flaps.state != 0f)
            coeffs = virtualAirfoil.Coefficients(angleOfAttack, flaps.MainSurface.Design, aircraft.hydraulics.flaps.state * flaps.TotalOverlap);
        else
            coeffs = virtualAirfoil.Coefficients(angleOfAttack);
