...
public class HydraulicSystemEditor : Editor
{
    ...
    public float visualizeHydraulics;
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (showMechanism)
        {
            ...
            float oldValue = visualizeHydraulics;
            visualizeHydraulics = EditorGUILayout.Slider(new GUIContent("Visualize In Editor"),visualizeHydraulics, 0f, 1f);

            if(visualizeHydraulics != oldValue)
            {
                hydraulic.SetInstant(visualizeHydraulics);
                hydraulic.animator.Update(100f);
            }
