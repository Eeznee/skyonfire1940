...
public class AircraftMarker : Marker
{
    ...
    protected override string TextToShow(float distance)
    {
        ...
        {
            txt += "\n" + "Sqdr " + (targetAircraft.SquadronId + 1);
            txt += targetAircraft.placeInSquad == 0 ? " Leader" : " Wing " + targetAircraft.placeInSquad;
        }
        else if (targetAircraft && targetAircraft.crew[0] != null && SofSettingsSO.CurrentSettings.showAIBehavior)
        {
            txt += "\n" + targetAircraft.crew[0].Seat.Action;
        }
        return txt;
    }
}