...
public class AircraftMarker : DynamicUI
{
    ...
    {
        if (PlayerManager.player.aircraft == null || GameManager.gameUI == GameUI.CamerasEditor) return true;
        bool playerTag = target.tag == PlayerManager.player.sofObj.tag;
        return target != PlayerManager.player.aircraft && PlayerPrefs.GetInt(playerTag ? "AlliedMarkers" : "EnnemiesMarkers", 1)  == 1;
    }
    ...
    private void LateUpdate()
    {
        ...
        //Rect
        Vector3 screenPoint = PlayerCamera.cam.WorldToScreenPoint(target.transform.position);
